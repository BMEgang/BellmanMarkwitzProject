...
def simulated_annealing(data, initial_temp=1.0, final_temp=0.01, alpha=0.9, max_iterations=100):
    num_stocks = data.shape[1]
    current_solution = np.random.dirichlet(np.ones(num_stocks))
    current_fitness = calculate_fitness(current_solution, data.pct_change().dropna().mean())
    temp = initial_temp

    for _ in range(max_iterations):
        while temp > final_temp:
            new_solution = np.random.dirichlet(np.ones(num_stocks))
            new_fitness = calculate_fitness(new_solution, data.pct_change().dropna().mean())
...
# 使用模拟退火算法优化投资组合
best_weights = simulated_annealing(train)
...
plt.savefig(f"{image_folder}/annealing_Optimized_Portfolio.png")
...
csv_save_path = os.path.join(data_folder, "annealing_Optimized_Portfolio.csv")
portfolio_df.to_csv(csv_save_path, index=False)

# 打印模拟退火算法优化后的权重
print("Optimized Weights from Simulated Annealing:")
print(best_weights)
