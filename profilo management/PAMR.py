...
portfolio_values_pamr, daily_weights_pamr = simulate_investment_pamr(trade, initial_capital, weights, epsilon, aggressiveness)
...
selected_weights = daily_weights_pamr[selected_dates]
average_weights = np.mean(selected_weights, axis=0)

# 获取特定时间段的初始和最终资本
initial_capital_period = portfolio_values_pamr[trade.index[1:].searchsorted(start_date)]
final_capital_period = portfolio_values_pamr[trade.index[1:].searchsorted(end_date) - 1]
...
# 计算投资组合的每日收益率
portfolio_daily_returns = np.diff(portfolio_values_pamr) / portfolio_values_pamr[:-1]

# 生成一个与 portfolio_daily_returns 长度匹配的布尔索引数组
