...
def simulate_investment_up(data, initial_capital, num_samples=100, update_frequency='monthly'):
    ...
    for i in range(1, len(data)):
        if update_frequency == 'monthly' and i % 30 == 0 or update_frequency == 'quarterly' and i % 90 == 0 or i == 1:
            ...
            previous_day_returns = data.iloc[i - 1] / data.iloc[i - 2] - 1
            current_best_weights = max(weights_combinations, key=lambda w: np.dot(previous_day_returns, w))
            weights_over_time.append(current_best_weights)  # 存储权重
...
portfolio_values_up, monthly_weights_up = simulate_investment_up(trade, initial_capital, num_samples=100, update_frequency='monthly')
...
# 创建一个与 monthly_weights_up 长度匹配的月度布尔索引
monthly_index = pd.date_range(start=trade.index[1], end=trade.index[-1], freq='M')
monthly_selected_period_adjusted = []

for date in monthly_index:
    if date >= pd.to_datetime(start_date) and date <= pd.to_datetime(end_date):
        monthly_selected_period_adjusted.append(True)
    else:
        monthly_selected_period_adjusted.append(False)

# 由于 monthly_weights_up 长度只有24，我们需要截断 monthly_selected_period_adjusted
monthly_selected_period_adjusted = monthly_selected_period_adjusted[:len(monthly_weights_up)]

# 应用调整后的月度布尔索引
selected_weights = monthly_weights_up[monthly_selected_period_adjusted]

# 计算平均配置比例
average_weights = selected_weights.mean(axis=0)

# 计算收益
initial_capital_period = portfolio_values_up[trade.index[1:].searchsorted(start_date)]
final_capital_period = portfolio_values_up[trade.index[1:].searchsorted(end_date) - 1]
...
# 计算投资组合每日收益率
portfolio_returns_up = np.diff(portfolio_values_up) / portfolio_values_up[:-1]

# 筛选特定时间段内的每日收益率
selected_returns_up = portfolio_returns_up[(trade.index[1:-1] >= start_date) & (trade.index[1:-1] <= end_date)]

# 计算波动率（标准差）
volatility_up = np.std(selected_returns_up)

# 输出风险（波动率）
print("Volatility from 2017-11 to 2017-12: {:.2%}".format(volatility_up))