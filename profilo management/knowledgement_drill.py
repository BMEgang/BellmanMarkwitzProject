...
# 筛选2017年11月至2017年12月的预测权重数据
start_date = "2017-11-01"
end_date = "2017-12-01"
selected_weights = trade_predictions_df.loc[start_date:end_date]

# 计算平均权重
average_weights = selected_weights.mean()
...
# 计算绝对收益和比例收益
initial_capital_period = capital_curve_df.loc[start_date, 'Capital']
final_capital_period = capital_curve_df.loc[end_date, 'Capital']
...
# 计算资金曲线的每日收益率
capital_curve_series = pd.Series(capital_curve, index=trade_predictions_df.index)
daily_returns = capital_curve_series.pct_change().dropna()

# 筛选2017年11月至2017年12月的日收益率
selected_period_returns = daily_returns[start_date:end_date]

# 计算波动率（标准差）
volatility = selected_period_returns.std()
